...
down_revision: Union[str, None] = "55d3ebcad301"
...
def upgrade() -> None:
    ...
    connection.execute(
        sa.text(
            "DELETE FROM alembic_version WHERE version_num = 'regenerate_channel_snowflake_ids'"
        )
    )
    connection.execute(
        sa.text(
            "INSERT INTO alembic_version (version_num) VALUES ('regenerate_channel_snowflake_ids')"
        )
    )
    connection.commit()
