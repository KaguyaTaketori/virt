...
async def get_youtube_channel_info(input_str: str) -> dict | None:
    ...
    channel_id = await resolve_youtube_channel(input_str) # 改为 await
    if not channel_id:
        return None
    ...
    if settings.youtube_api_key:
        try:
            ...
            logger.warning(f"YouTube API Error for snippet: {e}")
...
async def get_channel_info_from_page(input_str: str, channel_id: str) -> dict | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            avatar_match = re.search(r'"avatar":{"thumbnails":\[{"url":"([^"]+)"', html_text)
            avatar_url = clean_youtube_url(avatar_match.group(1)) if avatar_match else None
...
async def get_channel_details_from_page(channel_id: str) -> dict | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            twitter_match = re.search(r'https?://(?:www\.)?(?:twitter|x)\.com/([a-zA-Z0-9_]+)', html_text)
            twitter_url = (
