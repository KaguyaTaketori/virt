...
class YouTubeSyncService:
    ...
    async def sync_channel_videos(
        ...
        progress: Optional[Any] = None,  # 接收 rich.progress.Progress 对象
        task_id: Optional[Any] = None    # 接收具体的 TaskID
    ) -> int:
        """
        同步频道视频的主逻辑。
        1. 确定 Playlist ID。
        ...
        # 如果提供了进度条，但不知道总量，可以先把 total 设为 None 显示脉冲动画
        if progress and task_id:
            progress.update(task_id, description=f"  ↳ [yellow]正在连接[/yellow] {channel.name[:10]}...", total=None)
        ...
        while True:
            ...
            # 如果 UU ID 报错，尝试 API 获取真实 Uploads Playlist ID (通常只在第一页尝试)
            if status != 200 and page_token is None:
                ...
                if not playlist_id:
                    logger.warning("无法获取频道 {} 的播放列表 ID", channel.name)
                    break
                # 重新尝试获取第一页
                video_ids, next_token, status = await self.api.fetch_playlist_page(
                    playlist_id
            ...
            for item in items:
                ...
                if progress and task_id:
                    progress.update(
                        task_id,
                        description=f"  ↳ [cyan]正在抓取[/cyan] {channel.name[:10]} (已获 {total_processed} 视频)",
                        advance=1 # 向前走一格
                    )

            # 每一页刷新一次数据库 Session，避免事务过大
            await session.flush()
        ...
        # 最终状态更新
        if progress and task_id:
            progress.update(task_id, description=f"  ↳ [green]完成[/green] {channel.name[:10]} (共 {total_processed})")
