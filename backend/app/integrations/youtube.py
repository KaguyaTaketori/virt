...
class YouTubeService:
    """
    YouTube 统一服务层。
    整合 API 客户端、解析器、同步逻辑，提供完整的 YouTube 数据访问能力。
    """

    def __init__(self, http_client: Optional[BaseAPIClient] = None):
    ...
    async def get_videos(
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                resp = await client.get(f"{YT_API_BASE}/search", params=search_params)
                if resp.status_code == 200:
                    data = resp.json()
                    items = data.get("items", [])
    ...
    async def sync_channel_videos(
        ...
        async with httpx.AsyncClient() as client:
            ...
            existing_video_ids = await self._load_existing_video_id_set(
                session, channel.id
            )
            new_count = 0
            ...
            while True:
                ...
                if status != 200 or not video_ids:
                    break

                batch_details = await self._fetch_video_details_batch(
                    client, api_key, video_ids
                )

                for item in batch_details:
                    video_data = self._parse_video_item(
                        channel.id, Platform.YOUTUBE.value, item
                    )
                    if video_data["video_id"] not in existing_video_ids:
            ...
            return new_count, len(existing_video_ids)
